...
def moon_auto():
    ...
    if age < 1.5:
        ...
        msg = "🌖月の光メッセージ：心を整えて穏やかに過ごそう。"

    send_line_message(ADMIN_ID, msg)
    log_message_to_supabase(ADMIN_ID, msg, "moon_auto")
    return f"✅ Moon phase: {age:.1f}"
...
def weekly_report():
    ...
    try:
        ...
        if not logs:
            ...
            report_lines = [f"{k}: {v}件" for k, v in type_count.items()]
            type_summary = "\n".join(report_lines)
            ...
            analysis_prompt = (
                "以下は今週の会話ログです。主要な相談テーマを3点以内でまとめ、"
                "運用改善の提案を2点、簡潔に教えてください。出力は120字以内で。"
            )
